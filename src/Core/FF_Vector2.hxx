...
namespace FF {
    ...
    public:
         ...
         * @param y [Y component]
         */
        explicit Vector2( const T __FF_IN x = static_cast<T>(0.0f), 
        ...
        /**
         * @brief [Method that set X and Y component]
         * 
         * @param x [X component]
         * @param y [Y component]
         */
        inline void     SetXY(const T __FF_IN x, const T __FF_IN y) noexcept;
         ...
         * @param vec [Vector]
         */
        inline void operator-=(const FF::Vector2<T>& __FF_IN vec) noexcept;
         ...
         * @tparam T   [Generic type]
         * @return     [Return value that represent result of dot product this and parmeter vector]
         */
        inline T                      DotProduct(const FF::Vector2<T>& __FF_IN vec) const noexcept;
        ...
        /**
          ...
          */
        inline FF::Vector2<T>         Normalize(void) const;
    ...
    template<typename T>
    inline void FF::Vector2<T>::SetXComponent(const T __FF_IN x) noexcept {
        this->m_x = x;
    }

    template<typename T>
    inline void FF::Vector2<T>::SetYComponent(const T __FF_IN y) noexcept {
        this->m_y = y;
    }

    template<typename T>
    inline void FF::Vector2<T>::SetXY(const T __FF_IN x, const T __FF_IN y) noexcept {
        this->m_x = x;
        this->m_y = y;
    }

    template<typename T>
    const inline T& FF::Vector2<T>::GetXComponent(void) const noexcept {
        return m_x;
    }

    template<typename T>
    const inline T& FF::Vector2<T>::GetYComponent(void) const noexcept {
        return m_y;
    }

    template<typename T>
    FF::Vector2<T> operator-(const FF::Vector2<T>& __FF_IN vec){
        return FF::Vector2<T>( -vec.m_x, 
                               -vec.m_y );
    }
    ...
    template<typename T>
    inline void FF::Vector2<T>::operator-=(const FF::Vector2<T>& __FF_IN vec) noexcept {
        this->m_x -= vec.m_x;
        this->m_x -= vec.m_y;
    }
    ...
    template<typename T>
    inline void FF::Vector2<T>::operator-=(const T& __FF_IN scalar) noexcept {
        this->m_x -= scalar;
        this->m_y -= scalar;
    }
    ...
    template<typename T>
    inline T FF::Vector2<T>::Magnitude(void) const noexcept {
    ...
    template<typename T>
    inline T FF::Vector2<T>::SquaredMagnitude(void) const noexcept {
    ...
    template<typename T>
    inline FF::Vector2<T> FF::Vector2<T>::Normalize(void) const {
